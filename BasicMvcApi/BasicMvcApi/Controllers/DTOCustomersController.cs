...
namespace BasicMvcApi.Controllers.DTO;
...
public class CustomersController : ControllerBase
{
	...
	public async Task<IEnumerable<CustomerSummary>> GetAllAsync(ICustomerRepository customerRepository)
	{
		...
		var customersSummary = customers
			   .Select(customer => new CustomerSummary(
						...
						customer.Contracts.Count(x => x.Status.State != WorkState.Completed)
					))
			;
	...
	// PUT dto/customers/5
	[HttpPut("{customerId}")]
	public async Task<ActionResult<CustomerDetails>> PutAsync(int customerId, [FromBody] UpdateCustomer input,
															  ICustomerRepository customerRepository)
	{
		...
		if (updatedCustomer == null) return Conflict();
