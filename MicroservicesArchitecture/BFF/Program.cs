...
app.MapGet(
		...
		(int productId, IWebClient client, CancellationToken cancellationToken)
			=> client.Catalog.FetchProductAsync(new Products.FetchOne.Query(productId), cancellationToken)
	);
...
app.MapGet(
		...
		async (IWebClient client, ICurrentCustomerService currentCustomer, CancellationToken cancellationToken) =>
		{
			...
			var basket = await client.Baskets.FetchCustomerBasketAsync(
								 new Baskets.FetchItems.Query(currentCustomer.Id),
								 cancellationToken
							 );
			...
			await Parallel.ForEachAsync(basket, cancellationToken, async (item, cancellationToken) =>
			{
				...
				var product = await client.Catalog.FetchProductAsync(
									  new Products.FetchOne.Query(item.ProductId),
									  cancellationToken
								  );
...
app.MapPost(
		"api/cart",
		async (UpdateCartItem item, IWebClient client, ICurrentCustomerService currentCustomer,
...
static async Task RemoveItemFromCart(UpdateCartItem item, IWebClient client, ICurrentCustomerService currentCustomer,
									 CancellationToken cancellationToken)
{
	try
	{
		var result = await client.Baskets.RemoveProductFromCart(
							 new Baskets.RemoveItem.Command(
									 currentCustomer.Id,
									 item.ProductId
								 ),
							 cancellationToken
...
static async Task AddOrUpdateItem(UpdateCartItem item, IWebClient client, ICurrentCustomerService currentCustomer,
								  CancellationToken cancellationToken)
{
	try
	{
	...
	catch (ValidationApiException ex)
	{
		...
		var result = await client.Baskets.UpdateProductQuantity(
							 new Baskets.UpdateQuantity.Command(
									 currentCustomer.Id,
									 item.ProductId,
									 item.Quantity
								 ),
