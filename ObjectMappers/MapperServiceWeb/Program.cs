...
app.MapGet("/products",
	async (IProductRepository productRepository, IMappingService mapper, CancellationToken cancellationToken) =>
	{
...
app.MapPost("/products/{productId:int}/add-stocks",
		async (int productId, AddStocksCommand command, StockService stockService, IMappingService mapper,
...
app.MapPost("/products/{productId:int}/remove-stocks",
		async (int productId, RemoveStocksCommand command, StockService stockService, IMappingService mapper,
			   CancellationToken cancellationToken) =>
		{
			try
			{
...
public class ProductMapper : IMapper<Product, ProductDetails>
{
	...
	{
		return new ProductDetails(entity.Id ?? default, entity.Name, entity.QuantityInStock);
	}
}
...
public class ExceptionsMapper : IMapper<ProductNotFoundException, ProductNotFound>,
								IMapper<NotEnoughStockException, NotEnoughStock>
{
	public NotEnoughStock Map(NotEnoughStockException exception)
	{
		return new NotEnoughStock(exception.AmountToRemove, exception.QuantityInStock, exception.Message);
	}

	public ProductNotFound Map(ProductNotFoundException exception)
	{
		return new ProductNotFound(exception.ProductId, exception.Message);
	}
}
